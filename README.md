...
This project implements a modular Analog-to-Digital Signal Converter (ADC) in MATLAB, converting analog signals (e.g., speech) into digital signal (e.g., WAV). It explores two main areas:
...
**DC Removal (HPF) Object:** This module generates an instance of an N-order Butterworth High-Pass Filter (HPF) to eliminate unwanted DC bias from the incoming signal. In DSP, DC offsets are not universally detrimental; for example, unipolar quantizers rely on an injected DC offset to lift the analog signal entirely above zero. However, this simulation implements a bipolar midtread quantizer, which requires the signal to swing symmetrically across its zero-crossing. In this architecture, a residual DC offset restricts the dynamic range and severely degrades the quantizer's operational accuracy, making this HPF stage critical. A 4<sup>th</sup> order Butterworth HPF was used to attenuate the DC bias, yielding the centered signal shown in Figure 3.
...
- **Peak Envelope Detector:** Chosen over RMS detection to ensure deterministic quantizer safety. By tracking the true peak rather than energy averages, the system reacts to transients, preventing hard-clipping and maintaining high fidelity signal mapping within the quantizer’s full-scale range.
...
**ADC (Sampler + Quantizer):** This module models the complete analog-to-digital conversion pipeline by mapping the conditioned continuous-time analog signal into a discrete-time, discrete-amplitude digital sequence. Designed with an Object-Oriented architecture, it clearly separates the sampling and quantization phases to allow for flexible, dynamic parameter injection (such as Sampling Frequency, Full-Scale Voltage, and Bit Depth). The system prioritizes a midtread uniform bipolar quantizer over a midrise architecture. The midtread approach was specifically selected because it provides a true "zero" representation level; when the analog input is zero or contains negligible noise, the digital output remains exactly zero, effectively preventing idle channel noise and limit cycle oscillation.

**Key Implementation Features:**
- **Dynamic Parameterization:** Avoids hardcoded values by injecting key hardware specifications ($F_s$, $V_{fs}$, and bit resolution $B$) upon instantiation. These parameters dynamically calculate the precise quantization step size ($\Delta = V_{fs} / 2^B$) and bipolar voltage limits.
- **Hardware-Realistic Clip Protection:** Integrates strict saturation logic (min/max bounding) aligned with the full-scale voltage range. This accurately simulates real-world hardware overflow and underflow scenarios, preventing erroneous out-of-bounds indexing during extreme signal peaks.
- **Explicit Quantization Error Extraction:** Automatically calculates and isolates the quantization noise alongside the digitized signal. This is critical for evaluating the system's noise floor and accurately calculating the Signal-to-Quantization-Noise Ratio (SQNR).
